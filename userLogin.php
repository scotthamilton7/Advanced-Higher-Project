...
<html>
    ...
    <body>
        <section class="centerBox" id="loginBox">            
            <ul id="tabSelect">
                <li><a href="userLogin.php" class="active" style="border-top-left-radius: 16px;">Personal</a></li>
                <li><a href="businessLogin.php" style="border-top-right-radius: 16px;">Business</a></li>
            </ul>
            ...
            <section class="loginForm">
                ...
                <form id="userLoginForm" action="userLogin.php" method="POST">
                    ...
                    <input type="text" id="email" name="userEmail" required maxlength="100"><br><br>
                    <label for="pword"> Password*</labe><br>
                    <input type="password" id="pword" name="password" required maxlength="30"><br><br>
                    <section class="flex">
                        <a class="flexChild50" href="userSignUp.php">Create Account</a>
        ...
        <?php
            ...
            if (ISSET($_POST['logInBtn'])) {
                ...
                $query = "SELECT * FROM user WHERE user.email = '$email' AND user.password = '$pword'";
                ...
                if ($result) {
                    ...
                    if ($row_count == 1) {
                        // Assign the userID and the account type to session variables
                        $row = mysqli_fetch_assoc($result);
                        $_SESSION['accountID'] = $row['userID'];
                        $_SESSION['accountType'] = "Personal";

                        // If a row is returned the details are correct, navigate to userHome.php
                        header("Location:userHome.php");
                    }
                    else {
                        // If no records are returned then details are incorrect or account type is business
                        echo "<p>Could not find your account, try:</p>
                            <ol>
                                <li><a href='businessLogin.php'>Log in as a business instead</a></li>
                                <li><a href='userSignUp.php'>Create a new account</a></li>
                            </ol>";
                    }
                }
                else {
